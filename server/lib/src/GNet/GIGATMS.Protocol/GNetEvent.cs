...
{
	[DebuggerNonUserCode]
	public class GNetEvent : IEvent
	{
		private GNetReply m_oReply;
		...
		[DebuggerNonUserCode]
		public GNetEvent(ref byte[] bBuffer)
		{
			int num = default(int);
			if (bBuffer != null)
			{
			...
			m_oReply = new GNetReply(ref bBuffer, 1, checked(num - 2));
		}

		[DebuggerNonUserCode]
		public GNetEvent(ref byte[] bBuffer, int iLength)
		{
			m_oReply = new GNetReply(ref bBuffer, iLength, bIsRxCheck: false);
		}
